  ...
  implements HtmlDocument.Visitor
{
  ...
  {
    for (int i = -1 + this.stack.size(); i >= 0; i--)
  ...
  {
    return (HTML.Element)this.stack.remove(-1 + this.stack.size());
  }
  ...
  class TableFixer
  {
    ...
    void seeEndTag(HtmlDocument.EndTag paramEndTag)
    {
      int i = 1;
      HTML.Element localElement = paramEndTag.getElement();
      if ((this.tables > 0) && (localElement.getType() == i))
      {
