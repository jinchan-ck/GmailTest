...
public class PagerTitleStrip extends ViewGroup implements ViewPager.Decor {
	...
	private static PagerTitleStripImpl IMPL = new PagerTitleStripImplBase();
	...
	public PagerTitleStrip(Context paramContext, AttributeSet paramAttributeSet) {
		...
		TypedArray localTypedArray1 = paramContext.obtainStyledAttributes(
				paramAttributeSet, ATTRS);
		...
		boolean bool = false;
		if (i != 0) {
			TypedArray localTypedArray2 = paramContext.obtainStyledAttributes(
					i, TEXT_ATTRS);
		...
		while (true) {
			this.mScaledTextSpacing = ((int) (16.0F * paramContext
	...
	protected void onAttachedToWindow() {
		...
		if (this.mWatchingAdapter != null)
			;
		for (PagerAdapter localPagerAdapter2 = (PagerAdapter) this.mWatchingAdapter
				.get();; localPagerAdapter2 = null) {
	...
	protected void onMeasure(int paramInt1, int paramInt2) {
		...
		int i2 = m - i1;
		int i3 = View.MeasureSpec
				.makeMeasureSpec((int) (0.8F * k), -2147483648);
		...
		setMeasuredDimension(k,
				Math.max(n, i1 + this.mCurrText.getMeasuredHeight()));
	}
	...
	void updateAdapter(PagerAdapter paramPagerAdapter1,
	...
	void updateText(int paramInt, PagerAdapter paramPagerAdapter) {
		...
		if (paramPagerAdapter != null) {
			...
			if (paramInt >= 1) {
				...
				if (paramPagerAdapter != null)
					localCharSequence1 = paramPagerAdapter
							.getPageTitle(paramInt - 1);
			}
		...
		label239: for (CharSequence localCharSequence2 = paramPagerAdapter
				.getPageTitle(paramInt);; localCharSequence2 = null) {
			...
			if (j < i) {
				...
				if (paramPagerAdapter != null)
					localCharSequence3 = paramPagerAdapter
							.getPageTitle(paramInt + 1);
			}
			...
			int n = View.MeasureSpec.makeMeasureSpec((int) (0.8F * k),
					-2147483648);
	...
	private class PageListener extends DataSetObserver implements
		...
		public void onAdapterChanged(PagerAdapter paramPagerAdapter1,
				PagerAdapter paramPagerAdapter2) {
			PagerTitleStrip.this.updateAdapter(paramPagerAdapter1,
					paramPagerAdapter2);
		}

		public void onChanged() {
			PagerTitleStrip.this.updateText(
					PagerTitleStrip.this.mPager.getCurrentItem(),
					PagerTitleStrip.this.mPager.getAdapter());
			boolean bool = PagerTitleStrip.this.mLastKnownPositionOffset < 0.0F;
			float f = 0.0F;
			if (!bool)
				f = PagerTitleStrip.this.mLastKnownPositionOffset;
			PagerTitleStrip.this.updateTextPositions(
					PagerTitleStrip.this.mPager.getCurrentItem(), f, true);
		}
	...
	static class PagerTitleStripImplIcs implements
			PagerTitleStrip.PagerTitleStripImpl {
		public void setSingleLineAllCaps(TextView paramTextView) {
 ...
 * Qualified Name: android.support.v4.view.PagerTitleStrip JD-Core Version:
 * 0.6.2
 */